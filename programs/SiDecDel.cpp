...
/* ------------------------------------------------------------------------ */
/*           SIMULTANEOUS DECIMATION OF A DELAUNAY TRIANGULATION            */
/* ------------------------------------------------------------------------ */
...
int main( int argc, char **argv )
{
   ...
   //
   // Choice of the next point set to be removed: error-driven or random
   //
   ...
   //
   if (interactive_mode)
      kdegree = AskIntegerAtLeast("Max degree of removable vertices (0 = no constraints)", 0);
   else
   ...
   if ( nextpt == RANDOM )
           T = new TDecRndDeBerg( kdegree, &MT );
		  //T=new TDecRndDelaunay( kdegree, &MT );
   else // nextpt == ERROR
          T = new TDecErrDeBerg( kdegree, &MT, errrecalc );
         //  T = new TDecErrDelaunay( kdegree, &MT, errrecalc );
