...
int main(int argc, char **argv)
{
  ...
  int a,b,c;        /* vertices of a triangle */
  ...
  for (j=0; j<n_col-1; j++)
  {
     a = i*n_col + j;       /* index of grid point i,j */
     b = (i+1)*n_col + j+1; /* index of grid point i+1,j+1 */
     c = (i+1)*n_col + j;   /* index of grid point i,j+1 */
     fprintf(f_out, "%d %d %d\n", a,c,b);
     c = i*n_col + j+1; /* index of grid point i+1,j */
     fprintf(f_out, "%d %d %d\n", a,b,c);
  }
  return 1;
}
